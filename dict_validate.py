...
class MazeConfig(BaseModel):
    ...
    seed: Optional[int] = Field(default=None, ge=0) # must be positive if provided

    @field_validator('entry', 'exit', mode='before')
    @classmethod
    def parse_coordinates(cls, value: Any) -> tuple[int, int]:
        """
        Parses 'x,y' string into a tuple of ints before Pydantic validates types.
    ...
    def validate_filename(cls, value: str) -> str:
        ...
        """
        if not value.strip():
            raise ValueError("output_file name cannot be empty or whitespaces!!")
        return value.strip()
    ...
    def validate_maze_logic(self) -> 'MazeConfig':
        ...
        if not (0 <= entry_x < self.width and 0 <= entry_y < self.height):
            raise ValueError(f"entry point {self.entry} is out of maze bounds!!")

        # 2- check exit bounds
        if not (0 <= exit_x < self.width and 0 <= exit_y < self.height):
            raise ValueError(f"exit point {self.exit} is out of maze bounds!!")
